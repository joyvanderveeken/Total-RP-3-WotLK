...
-- Create a unit ID from a unit name (for single-realm servers, no realm suffix needed)
-- This method ALWAYS returns a unit name string.
Utils.str.unitInfoToID = function(unitName)
	return unitName or "_";
end

-- Separates the unit name from a unit ID (no realm separation needed for single-realm)
Utils.str.unitIDToInfo = function(unitID)
	-- For single-realm servers, the unitID is just the character name
	return unitID, nil;
end

-- Create a unit ID based on a targetType (target, player, mouseover ...)
-- For single-realm servers, this just returns the character name
Utils.str.getUnitID = function(unit)
	local name = UnitName(unit);
	if not name or name:len() == 0 or name == UNKNOWNOBJECT then
		return nil;
	end
	return name;
end
