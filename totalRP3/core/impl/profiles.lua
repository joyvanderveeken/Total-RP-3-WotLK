...
local function decorateProfileList(widget, index)
	...
	for characterID, characterInfo in pairs(characters) do
		if characterInfo.profileID == id then
			local charactName = unitIDToInfo(characterID);
			listText = listText.."- |cff00ff00"..charactName.."|r\n";
			i = i + 1;
		end
	end
...
function TRP3_API.profile.init()
	...
	if not character.profileID or not profiles[character.profileID] then
		-- Detect if a profile with this player name already exists
		local available, profileID = isProfileNameAvailable(Globals.player);
		if not available and profileID then
			selectProfile(profileID);
		else
			selectProfile(createProfile(Globals.player));
		end
	else
